...
export default function CaseManagerPage() {
  ...
  async function saveStudent() {
    if (!studentForm.name.trim()) { setError("Student name is required."); return; }
    const supabase = getSupabase();
    if (!supabase) return;
  ...
  function cancelStudentEdit() { setEditingStudent(null); setStudentForm({ name: "", grade_level: "" }); }

  async function saveGoal() {
    if (!goalForm.student_id || !goalForm.goal_description.trim()) {
      setError("Student and goal description are required."); return;
    }
    const supabase = getSupabase();
    if (!supabase) return;
    setLoading(true); setError(null);
  ...
  function startEditGoal(g: Goal) { setEditingGoal(g); setGoalForm({ student_id: g.student_id, goal_description: g.goal_description }); }
  function cancelGoalEdit() { setEditingGoal(null); setGoalForm({ student_id: "", goal_description: "" }); }
  ...
  function parseCSV(text: string) {
    ...
    if (nameIdx === -1) { setCsvError("CSV must have a 'name' column. Optional: 'grade_level'."); return; }
  ...
  async function importCSV() {
    ...
    const supabase = getSupabase();
    if (!supabase) return;
    setLoading(true); setCsvError(null);
    const { error: e } = await supabase.from("students").insert(csvPreview);
    if (e) { setCsvError(e.message); } else {
  ...
  return (
    <main className="page">
      ...
      <div className="tab-bar">
        <button className={`tab-btn ${tab === "students" ? "tab-btn--active" : ""}`} onClick={() => setTab("students")}>Students</button>
        <button className={`tab-btn ${tab === "goals" ? "tab-btn--active" : ""}`} onClick={() => setTab("goals")}>IEP Goals</button>
        <button className={`tab-btn ${tab === "upload" ? "tab-btn--active" : ""}`} onClick={() => setTab("upload")}>CSV Upload</button>
      </div>
        ...
        <>
          <section className="card">
            ...
            <div className="form-grid">
              <div className="field">
                <label>Name *</label>
                <input type="text" value={studentForm.name}
                  onChange={(e) => setStudentForm({ ...studentForm, name: e.target.value })}
                  placeholder="Student full name" />
              </div>
              <div className="field">
                <label>Grade level</label>
                <input type="text" value={studentForm.grade_level}
            ...
            <div className="btn-row" style={{ marginTop: "18px" }}>
              ...
              {editingStudent && (
                <button className="button button-outline" type="button" onClick={cancelStudentEdit}>Cancel</button>
              )}
            </div>
          </section>
          ...
          <section className="card">
            ...
            {students.length === 0 ? (
              ...
              <table className="data-table">
                <thead><tr><th>Name</th><th>Grade</th><th>Goals</th><th>Actions</th></tr></thead>
                <tbody>
                  {students.map((s) => {
                    ...
                    return (
                      <tr key={s.id}>
                        <td>{s.name}</td>
                        <td>{s.grade_level || "—"}</td>
                        <td><span className="badge">{goalCount} goal{goalCount !== 1 ? "s" : ""}</span></td>
                        <td>
                          <div className="btn-row">
                            <button className="button button-sm button-outline" onClick={() => startEditStudent(s)}>Edit</button>
                            <button className="button button-sm button-danger" onClick={() => deleteStudent(s.id)}>Delete</button>
                          </div>
                        </td>
                      </tr>
                    );
        ...
        <>
          <section className="card">
            ...
            <div className="form-grid">
              <div className="field">
                <label>Student *</label>
                <select value={goalForm.student_id} onChange={(e) => setGoalForm({ ...goalForm, student_id: e.target.value })}>
                  <option value="">-- Select student --</option>
                  {students.map((s) => <option key={s.id} value={s.id}>{s.name} (Gr {s.grade_level})</option>)}
                </select>
              </div>
              <div className="field full">
                <label>Goal description *</label>
                <textarea value={goalForm.goal_description}
            ...
            <div className="btn-row" style={{ marginTop: "18px" }}>
              ...
              {editingGoal && (
                <button className="button button-outline" type="button" onClick={cancelGoalEdit}>Cancel</button>
              )}
            </div>
          </section>
          ...
          <section className="card">
            ...
            {goals.length === 0 ? (
              ...
              <table className="data-table">
                <thead><tr><th>Student</th><th>Goal</th><th>Actions</th></tr></thead>
                <tbody>
                  {goals.map((g) => (
                    <tr key={g.id}>
                      ...
                      <td>
                        <div className="btn-row">
                          <button className="button button-sm button-outline" onClick={() => startEditGoal(g)}>Edit</button>
                          <button className="button button-sm button-danger" onClick={() => deleteGoal(g.id)}>Delete</button>
                        </div>
                      </td>
                    </tr>
                  ))}
      ...
      {tab === "upload" && (
        <section className="card">
          ...
          <div className="info-banner" style={{ marginBottom: "20px" }}>
            ...
            <strong>Optional:</strong> <code>grade_level</code><br />
          ...
          <div className="field" style={{ marginBottom: "20px" }}>
            ...
            <input ref={fileInputRef} type="file" accept=".csv,text/csv"
              onChange={handleFileChange} style={{ padding: "8px 0" }} />
          </div>
          ...
          {csvPreview.length > 0 && (
            <>
              ...
              <table className="data-table" style={{ marginBottom: "20px" }}>
                <thead><tr><th>#</th><th>Name</th><th>Grade level</th></tr></thead>
                <tbody>
                  {csvPreview.map((row, i) => (
                    <tr key={i}><td>{i + 1}</td><td>{row.name}</td><td>{row.grade_level || "—"}</td></tr>
                  ))}
                </tbody>
              </table>
